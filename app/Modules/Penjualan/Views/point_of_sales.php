        ...
        <v-dialog v-model="kasir" fullscreen hide-overlay transition="dialog-bottom-transition" scrollable persistent>
            <v-card>
                <v-toolbar color="<?= $navbarColor; ?>" <?= ($navbarColor == 'white' ? 'light' : 'dark'); ?> class="mb-3">
                    <v-btn icon dark link href="<?= base_url('penjualan'); ?>">
                                ...
                                <v-data-table :height="height1" :headers="tbkeranjang" :fixed-header="true" :items="dataKeranjang" :items-per-page="-1" item-key="id_keranjang" :loading="loading" loading-text="<?= lang('App.loadingWait'); ?>" :footer-props="{itemsPerPageText: ''}" style="overflow-y: auto;">
                                    <template v-slot:item="{ item }">
                                        <tr>
                                            ...
                                            <td>
                                                <h3 class="font-weight-medium mb-1">{{item.nama_barang}}</h3>
                                            ...
                                            <td width="150">
                                                <a class="font-weight-bold text-h6 primary--text" @click="editItem(item)">
                                                    {{RibuanLocaleNoRp(item.jumlah)}}
                                                </a><br />
                                                <?php if ($ppn > 0) : ?>
                                                    PPN: {{RibuanLocaleNoRp(item.ppn)}}
                                                <?php endif; ?>
                                            </td>
                                    ...
                                    <template slot="footer.prepend">
                                        Rp<h1 style="font-size: 1.6rem;">{{RibuanNoRp(subtotal + sumTotalPPN('ppn'))}}</h1>&nbsp;<p>DPP + PPN</p>
                                    ...
                                    <v-row>
                                        <v-col cols="12" sm="5" <?php if ($cashierpayPos == 'left') { ?>order="first" <?php } else { ?>order="last" <?php } ?>>
                                            <div v-if="metodeBayar == 'cash'">
                                                <v-btn small color="primary" v-on:click="bayar += total" :disabled="bayar == total">{{ total.toLocaleString('id-ID') }}*</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar = 0" outlined>0</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 500" outlined>500</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 1000" outlined>1k</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 2000" outlined>2k</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 5000" outlined>5k</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 10000" outlined>10k</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 20000" outlined>20k</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 50000" outlined>50k</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 100000" outlined>100k</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 200000" outlined>200k</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 500000" outlined>500k</v-btn>
                                                <v-btn small class="ma-1" v-on:click="bayar += 1000000" outlined>1jt</v-btn>
                                            </div>
                                        ...
                                        <v-col cols="12" sm="7">
                                            <v-row>
                                                <v-col>
                                                    <v-text-field v-model="diskon" :label="'Diskon (Rp) ' + Ribuan(diskon)" type="number" min="0" outlined dense :rules="[rules.required]" @focus="$event.target.select()" :error-messages="diskonError" :suffix="diskonPersen.toFixed() + '%'"></v-text-field>
                                                ...
                                                <v-col>
                                                    <v-text-field v-model="bayar" ref="bayar" :label="'Bayar (F2) ' + Ribuan(bayar)" type="number" min="0" outlined dense @focus="$event.target.select()" v-on:keydown.enter="savePenjualan" :hint="'*<?= lang('App.rounding'); ?>: ' + pembulatan" persistent-hint></v-text-field>
                                                    <v-text-field v-model="kembali" :label="'Kembali/Kurang ' + Ribuan(kembali)" readonly outlined dense filled></v-text-field>
                                                </v-col>
                                            </v-row>
                                        </v-col>
                                    </v-row>
                                    ...
                                    <tr class="product-card">
                                        <td>
                                            <v-list-item class="ma-n5 pa-n5" two-line @click="saveKeranjang(item)" title="<?= lang('App.addtoCart'); ?>" alt="<?= lang('App.addtoCart'); ?>">
                                                ...
                                                <v-list-item-content>
                                                    <p class="font-weight-medium">{{item.nama_barang}}</p><br />
                                                    <span class="text-caption">
                                                        <?= lang('App.stock'); ?>: <strong>{{item.stok}}</strong> &bull; <?= lang('App.unit'); ?>: {{item.satuan_barang}}/{{item.satuan_nilai}} &bull; <?= lang('App.prc'); ?>: <strong>
                                                            ...
                                                            <span v-else>{{ Ribuan(item.harga_jual) }}</span></strong>
                                                        &nbsp;
                                                        <v-chip color="success" outlined label x-small v-show="item.jumlah_min_grosir > 0"><strong><?= lang('App.wholesale'); ?></strong></v-chip>
                                                    </span>
                                                </v-list-item-content>
                                        ...
                                        <td width="30%">{{item.kode_barang}}<br />{{item.barcode}}<br />SKU {{item.sku ?? "-"}}</td>
                    ...
                    <v-row>
                        <v-col cols="12" sm="6">
                            ...
                            <div v-for="item in itemPenjualan" :key="item.id_itempenjualan">
                                {{item.nama_barang}}<br />
                                {{item.qty}} ({{item.satuan}}) x {{RibuanLocaleNoRp(item.harga_jual_grosir == 0 ? item.harga_jual: (item.qty >= item.jumlah_min_grosir ? item.harga_jual_grosir:item.harga_jual))}}
                                <span class="float-right">{{RibuanLocaleNoRp(item.jumlah)}}</span>
                            </div>
                            <v-divider></v-divider>
                            <div>
                                DPP ({{jmlItemNota}} item): <span class="float-right">{{RibuanLocaleNoRp(subtotalNota ?? "0")}}</span><br />
